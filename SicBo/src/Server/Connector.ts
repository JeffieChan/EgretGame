module Server{
    ...
    export class Connector{
        ...
        public static startReconnectTimer(sender:any){
            if(!Connector.timer){
                Connector.timer = new egret.Timer(5000,100);
        ...
        public static reConnectToGameServer(serverReconnected:(any)=>void,sender:any){
            ...
            if(Connector.gameClient.State == ClientState.Disconnected){
                Connector.gameClient.reConnect(
                    ...
                    function(){
                        Connector.gameClient.reEnter(CommonUtils.AccountUtils.getAccountToken(),sender);
                    }
                );
            }
        ...
        public static onReloadGameDataSuccess(data:any, sender:any){
            if(Connector.timer){
                Connector.timer.stop();
            }
